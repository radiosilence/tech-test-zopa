...
export const getTransactions = (state: RootState) => getAccountState(state).transactions

export const getSent = createSelector(
  [getTransactions],
  (transactions: TransactionModel[]) =>
    transactions.reduce((acc: Decimal, t: TransactionModel) => acc.plus(t.amount), new Decimal(0)),
)

export const getAvailable = createSelector(
  [getBalance, getSent],
  (balance: Decimal, sumTransactions: Decimal) => balance.minus(sumTransactions),
)
